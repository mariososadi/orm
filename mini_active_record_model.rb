module MiniActiveRecord
  ...
  class Model
    ...
    def self.name_class
        @name_class= self.to_s.downcase + "s"
      end
    ...
    def self.all
      MiniActiveRecord::Model.execute("SELECT * FROM #{self.name_class}").map do |row|
    self.new(row)
    end
  end
    ...
    MiniActiveRecord::Model.execute("SELECT * FROM #{self.name_class} WHERE #{query}", *args).map do |row|
      self.new(row)
    end
  end
  ...
  def insert!
    ...
    insert_sql = "INSERT INTO #{self.class.name_class} (#{fields.join(',')}) VALUES (#{marks})"
  ...
  def update!
    ...
    update_sql = "UPDATE #{self.class.name_class} SET #{update_clause} WHERE id = ?"
